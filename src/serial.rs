...
use stm32::{RCC, USART1, USART2, USART3, USART4};

use gpio::gpioa::{PA0, PA1, PA10, PA14, PA15, PA2, PA3, PA9};
use gpio::gpiob::{PB6, PB7, PB10, PB11};
use gpio::gpioc::{PC4, PC5, PC10, PC11};
use gpio::{Alternate, AF0, AF1, AF4};
use rcc::Clocks;
use time::Bps;
...
impl Pins<USART4> for (PA0<Alternate<AF4>>, PA1<Alternate<AF4>>) {}
impl Pins<USART4> for (PC10<Alternate<AF0>>, PC11<Alternate<AF0>>) {}
...
impl<PINS> Serial<USART3, PINS> {
    pub fn usart1(usart: USART3, pins: PINS, baud_rate: Bps, clocks: Clocks) -> Self
    where
        PINS: Pins<USART3>,
    {
        // NOTE(unsafe) This executes only during initialisation
...
/// USART4
impl<PINS> Serial<USART4, PINS> {
    pub fn usart1(usart: USART4, pins: PINS, baud_rate: Bps, clocks: Clocks) -> Self
    where
        PINS: Pins<USART4>,
    {
        // NOTE(unsafe) This executes only during initialisation
    ...
    pub fn release(self) -> (USART4, PINS) {
        (self.usart, self.pins)
    }
}

impl hal::serial::Read<u8> for Rx<USART4> {
    type Error = Error;

    fn read(&mut self) -> nb::Result<u8, Error> {
        // NOTE(unsafe) atomic read with no side effects
        let isr = unsafe { (*USART4::ptr()).isr.read() };
        ...
        Err(if isr.pe().bit_is_set() {
            ...
            // NOTE(read_volatile) see `write_volatile` below
            return Ok(unsafe { ptr::read_volatile(&(*USART4::ptr()).rdr as *const _ as *const _) });
        } else {
            nb::Error::WouldBlock
        })
    }
...
impl hal::serial::Write<u8> for Tx<USART4> {
    type Error = Void;
    ...
    fn write(&mut self, byte: u8) -> nb::Result<(), Self::Error> {
        ...
        if isr.txe().bit_is_set() {
            // NOTE(unsafe) atomic write to stateless register
            // NOTE(write_volatile) 8-bit write that's not possible through the svd2rust API
            unsafe { ptr::write_volatile(&(*USART4::ptr()).tdr as *const _ as *mut _, byte) }
            Ok(())
        } else {
            Err(nb::Error::WouldBlock)
        }
